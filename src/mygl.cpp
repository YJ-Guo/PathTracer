...
MyGL::MyGL(QWidget *parent)
    : GLWidget277(parent),
      ...
      recursionLimit(5),
      numPhotons(500000), photonSearchRadius(0.25f),
...
void MyGL::RenderScene()
{
    ...
    // For Photon Mapper, need to generate all photons first
    if (integratorType & Photon_Mapper) {
        GeneratePhotons();
    }
...
void MyGL::GeneratePhotons() {
    // Use 16 threads to generate photons
    int ThreadPhotonNum = numPhotons / 8;
    ...
    for(int i = 0; i < 8; ++i)
    {
        // For the last task, should add the residue
        int curThreadNum = (i == 7) ? ThreadPhotonNum + remainder: ThreadPhotonNum;
        int seed = i;
        PhotonMapper* pm = new PhotonMapper(curThreadNum, &((*photonSets)[i]), &((*causticPhotonSets)[i]), &scene, sampler->Clone(seed), recursionLimit);
    ...
    // Put all the photons generated from different threads into one vector
    for(std::vector<Photon>& ps : *photonSets)
    {
        allPhotons->insert(allPhotons->end(), ps.begin(), ps.end());
    }
    delete photonSets;
    ...
    std::cout << "Buiding k-d tree" << std::endl;
    kdTree.build(allPhotons);
    delete allPhotons;

    // Construct a caustic k-d tree:
    std::cout << "Building caustic k-d tree" << std::endl;
    causticKDTree.build(allCausticPhotons);
    delete allCausticPhotons;
}
