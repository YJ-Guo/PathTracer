...
bool Sphere::Intersect(const Ray &ray, Intersection *isect) const
{
    ...
    if(t >= 0)
    {
        ...
        // Store the two intersection point in the intersection
        isect->CSGT1 = (-B - sqrt(discriminant))/(2*A);
        isect->CSGT2 = (-B + sqrt(discriminant))/(2*A);
        return true;
    }
    return false;
}
...
float Sphere::Pdf(const Intersection &ref, const Vector3f &wi) const {
    Point3f pCenter(transform.T() * glm::vec4(0, 0, 0, 1));
    Point3f pOrigin = ref.point + ref.normalGeometric * 1e-6f;
    if (glm::distance2(pOrigin, pCenter) <= 1.f) { // r^2 is 1
        return Shape::Pdf(ref, wi);
    }
    // Is usually r^2, but we assume sphere's radius is 1
    float sinThetaMax2 = 1.f / glm::distance2(ref.point, pCenter);
