...
class Light
{
  public:
    ...
    virtual Color3f Sample_Le(const Point2f &u1, const Point2f &u2, Ray *ray, Normal3f *nLight, Float *pdfPos, Float *pdfDir){return Color3f(0.f);}

    virtual Photon Sample_Photon(std::shared_ptr<Sampler> sampler, int numPhotons) const { return Photon(); }
