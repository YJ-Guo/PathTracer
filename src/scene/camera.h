...
public:
    ...
    virtual Ray RaycastNDC(float ndc_x, float ndc_y); //Creates a ray in 3D space given a 2D point in normalized device coordinates.
...
public:
    ...
    // Add three attributes for thin lens camera
    float mFocalLength;
    float mLensRadius;
    Sampler mSampler;
};
