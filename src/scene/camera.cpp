...
Ray Camera::Raycast(const Point2f &pt)
{
...
ThinLensCamera::ThinLensCamera(const ThinLensCamera &c):
    fovy(c.fovy),
    width(c.width),
    height(c.height),
    near_clip(c.near_clip),
...
void ThinLensCamera::RecomputeAttributes() {
    ...
    float len = mFocalLength;

    aspect = width/(float)height;
    // V and H calculation is the same as regular Camera
    V = up * len * tan_fovy;
    H = right * len * aspect * tan_fovy;
}

Ray ThinLensCamera::RaycastNDC(float ndc_x, float ndc_y) {
    // Follow Adam's Q&A code logic
    Point3f focalRef = eye + look * mFocalLength;
    Point3f targetPoint = focalRef + V * ndc_y + H * ndc_x;
    // Get the sample point position on a disc from square
    Point3f samplePoint = WarpFunctions::squareToDiskConcentric(mSampler.Get2D());
    Point3f rayOrigin = eye + up * mLensRadius * samplePoint.y  + right * mLensRadius * samplePoint.x;

    return Ray(rayOrigin, glm::normalize(targetPoint - rayOrigin));
}
