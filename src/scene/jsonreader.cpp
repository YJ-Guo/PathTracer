    ...
    if(file.open(QIODevice::ReadOnly))
    {
        ...
        foreach(const QJsonValue &frame, frames) {
            ...
            if(sceneObj.contains(QString("camera"))) {
                ...
                // Use this line for regual camera
                scene.SetCamera(LoadCamera(camera));
                // Use this line for Thin lens camera
                //scene.SetThinLensCamera(LoadThinLensCamera(camera));
            }
...
bool JSONReader::LoadGeometry(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives)
{
    ...
    {
        std::cout << "01Could not parse the geometry!" << std::endl;
        return NULL;
    }
...
bool JSONReader::LoadLights(QJsonObject &geometry, QMap<QString, std::shared_ptr<Material>> mtl_map, const QStringRef &local_path, QList<std::shared_ptr<Primitive>> *primitives, QList<std::shared_ptr<Light>> *lights)
{
    ...
    {
        std::cout << "02Could not parse the geometry!" << std::endl;
        return NULL;
    }
    ...
    if(QString::compare(lgtType, QString("DiffuseAreaLight")) == 0)
    {
        ...
        std::cout << "03Could not parse the geometry!" << std::endl;
        return NULL;
    }
...
bool JSONReader::LoadMaterial(QJsonObject &material, const QStringRef &local_path, QMap<QString, std::shared_ptr<Material> > *mtl_map)
{
    ...
    else if(QString::compare(type, QString("TransmissiveMaterial")) == 0)
    {
        ...
        float roughness = 0.f;
        // If the material has roughness term
        if (material.contains(QString("roughness"))) {
            roughness = material["roughness"].toDouble();
        }
        if(material.contains(QString("roughnessMap"))) {
        ...
        auto result = std::make_shared<TransmissiveMaterial>(Kt, roughness, eta, roughnessMap, textureMap, normalMap);
        mtl_map->insert(material["name"].toString(), result);
    }
    ...
    else if (QString::compare(type, QString("MetalMaterial")) == 0)
    {
        ...
        float roughness = material["roughness"].toDouble();
        auto result = std::make_shared<MetalMaterial>(eta, k, roughness);
        mtl_map->insert(material["name"].toString(), result);
    }
    else
    {
...
ThinLensCamera JSONReader::LoadThinLensCamera(QJsonObject &thinlensCamera)
{
    ThinLensCamera result;
    if(thinlensCamera.contains(QString("target"))) result.ref = ToVec3(thinlensCamera["target"].toArray());
    if(thinlensCamera.contains(QString("eye"))) result.eye = ToVec3(thinlensCamera["eye"].toArray());
    if(thinlensCamera.contains(QString("worldUp"))) result.world_up = ToVec3(thinlensCamera["worldUp"].toArray());
    if(thinlensCamera.contains(QString("width"))) result.width = thinlensCamera["width"].toDouble();
    if(thinlensCamera.contains(QString("height"))) result.height = thinlensCamera["height"].toDouble();
    if(thinlensCamera.contains(QString("fov"))) result.fovy = thinlensCamera["fov"].toDouble();
    if(thinlensCamera.contains(QString("nearClip"))) result.near_clip = thinlensCamera["nearClip"].toDouble();
    if(thinlensCamera.contains(QString("farClip"))) result.far_clip = thinlensCamera["farClip"].toDouble();
    if(thinlensCamera.contains(QString("focalLength"))) result.mFocalLength = thinlensCamera["focalLength"].toDouble();
    if(thinlensCamera.contains(QString("lensRadius"))) result.mLensRadius = thinlensCamera["lensRadius"].toDouble();
